...
int WriteCenterline(vtkPolyData* centerline, string centerlinePath)
{
			...
			centerlinePathInfo.suffix().toStdString() == "VTK"))
	{
		...
		vtkSmartPointer<vtkPolyData> linedata = branchExtractor->GetOutput();
		ofstream ofs;
		ofs.open(centerlinePath+".pcd", ios::out);
		ofs << "# .PCD v0.7 - Point Cloud Data file format\n"
			...
			<< "TYPE F F F\n"
			<< "COUNT 1 1 1\n"
			<< "WIDTH " << linedata->GetNumberOfPoints() <<"\n"
			<< "HEIGHT 1\n"
			<< "VIEWPOINT 0 0 0 1 0 0 0\n"
			<< "POINTS "<< linedata->GetNumberOfPoints()<<"\n"
			<< "DATA ascii\n";
		for (int i = 0; i < linedata->GetNumberOfPoints(); i++) {
			double* curpoint = linedata->GetPoint(i);
			ofs << curpoint[0] << "," << curpoint[1] << "," << curpoint[2] << "\n";
		}
...
int ReadSurface(string surfacePath, vtkPolyData* surface)
{
	...
	//去掉无用点 将合并重复点（在指定的容差内并且如果启用），消除未在任何单元格中使用的点
...
int centerline_extraction_Api(string intputfile, string capfile, string outcenterline) {
	...
	if (!ExtractCenterline(intputfile, inputSurface, cappedSurface, centerline, cylindricalShape))
		return 0;
	else
	{
		cout << "Finish Centerline extraction" << endl;
		// write the capped file
		if (!WriteCappedSurface(cappedSurface, capfile))
			return 0;
		if (!WriteCenterline(centerline, outcenterline))
			return 0;
